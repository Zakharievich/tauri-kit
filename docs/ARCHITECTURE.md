...
- **Роль:** единственная ответственность — выдача одноразовых JWT-токенов
  LiveKit. Не хранит данные пользователей, чат или историю сессий.
...
8. Агент (если был активен) по завершении сессии формирует финальный
   ...
   успевает опубликовать до дисконнекта — зависит от таймингов сессии).
        │
9. TranscriptPage отображает useTranscriptStore (сегменты + саммари, либо
   пустой стейт, если транскрипта не было).
        │
...
6. **Приём на фронтенде.** `hooks/useTranscription.ts` подписан на
   ...
   будущих изменений протокола) и кладёт `payload.transcript`/
   `payload.summary` в `useTranscriptStore`.

...
Скрипт `deploy.sh` выполняет всё, что делается на VPS (Шаги 2–8 ниже): пакеты, firewall, Docker, сетевой фикс, генерацию ключей и `.env`, запуск стека и проверку. Скрипт **идемпотентный** — его можно запускать повторно.

**Сначала** создайте DNS A‑запись (см. [Шаг 1](#шаг-1-домен-dns)) — домен должен указывать на IP сервера. Затем на VPS (**Ubuntu/Debian**, от root или через `sudo`) — один из двух путей:

**Вариант 1 — клонировать и запустить (рекомендуется, весь код виден до запуска):**
```bash
...
```bash
curl -fsSL https://raw.githubusercontent.com/Zakharievich/tauri-kit/main/bootstrap.sh | sudo bash -s -- --domain conf.example.com
```
> ⚠️ `curl … | sudo bash` выполняет скачанный скрипт с правами root. Запускайте так только доверенный источник; при сомнениях используйте Вариант 1 и просмотрите `deploy.sh` перед запуском.

Полезные флаги: `--force` (перегенерировать `.env`), `--skip-network-fix` (не трогать сетевую конфигурацию), `--yes` (неинтерактивно). Полный список — `bash deploy.sh --help`.

По завершении скрипт напечатает «URL сервера» вида `https://conf.example.com` — его укажите в клиенте. Опциональный STT‑агент скриптом пока не ставится (см. Шаг 9 ниже).

**Что осталось вручную** (принципиально вне скрипта): DNS A‑запись (Шаг 1, у регистратора) и сборка/установка десктоп‑клиента (Шаги 10–11, на машинах пользователей).

Пошаговая ручная установка ниже — на случай, если хотите контролировать каждый шаг или разбираетесь с проблемой (раздел [Диагностика](#диагностика-частых-проблем)).
...
- macOS: `src-tauri/target/release/bundle/dmg/*.dmg` (для дистрибуции за пределами своей машины нужна подпись Apple — см. ниже)

### macOS: ошибка «Приложение повреждено и не может быть открыто»

Причина: сборка **не подписана** сертификатом Apple Developer ID и **не нотаризована**. При скачивании `.dmg`/`.app` macOS ставит атрибут карантина (`com.apple.quarantine`), и Gatekeeper блокирует запуск, показывая «приложение повреждено». Файл не повреждён — это защита Gatekeeper.

**Как запустить (любой из способов):**

1. Снять карантин в терминале (замените путь на свой):
   ```bash
   xattr -cr /Applications/TauriKit.app
   ```
2. Правый клик (Ctrl+клик) по `TauriKit.app` → **«Открыть»** → в диалоге ещё раз **«Открыть»**. Достаточно сделать один раз.
3. **Системные настройки → Конфиденциальность и безопасность** → внизу, рядом с сообщением о заблокированном приложении, нажать **«Всё равно открыть»**.

После первого запуска приложение открывается как обычно. При первом звонке macOS запросит доступ к **камере** и **микрофону** — нужно разрешить.

**Полноценное решение (требует платного аккаунта Apple Developer, $99/год):** подписать приложение Developer ID (`codesign`) и нотаризовать через `notarytool`, после чего Gatekeeper перестанет блокировать скачанные сборки у всех пользователей. Конфигурация к этому подготовлена (`src-tauri/entitlements.plist`, блок `bundle.macOS` в `tauri.conf.json`); при наличии аккаунта в CI (`.github/workflows/ci.yml`, job `build-tauri`) добавляются секреты `APPLE_ID`, `APPLE_PASSWORD` (app-specific), `APPLE_TEAM_ID`, `APPLE_CERTIFICATE`, `APPLE_CERTIFICATE_PASSWORD` и `APPLE_SIGNING_IDENTITY`, которые `tauri-action` использует для подписи и нотаризации.

## Шаг 11. Использование клиента

Есть два сценария на главном экране:

**Создать новую комнату (хост):**
- **URL сервера** — базовый URL token‑server: `https://conf.example.com`. Поле запоминается между запусками.
- **Ваше имя** — уникальный идентификатор участника (попадает в JWT).
- **Шифрование (E2EE)** — опционально; ключ создаётся у хоста локально. При включённом E2EE транскрипция недоступна.
- **Транскрибация речи** — опционально; работает только если на сервере установлен и запущен Python‑агент. Взаимоисключает с E2EE.

Имя комнаты генерируется автоматически — вводить его не нужно. После подключения нажмите в комнате кнопку **«Добавить участника»** (иконка с человечком и плюсом): в буфер обмена скопируется **ссылка‑приглашение**, содержащая URL сервера, имя комнаты, флаги шифрования/транскрибации и E2EE‑ключ (если задан).

**Присоединиться по ссылке:**
- Вставьте полученную ссылку‑приглашение в поле **«Присоединиться по ссылке»** — URL сервера, комната и настройки заполнятся автоматически.
- Введите **своё имя** и нажмите **«Подключиться»**.

> ⚠️ Ссылка‑приглашение содержит E2EE‑ключ, поэтому любой, у кого есть ссылка, сможет войти и расшифровать сессию. Передавайте её только доверенным участникам по защищённому каналу.

По завершении сессии, если была включена транскрибация, откроется экран транскрипта, где кнопка сохранения пишет `.txt` в папку Documents; иначе кнопка **«Выйти»** (красный крестик) сразу возвращает на главный экран.

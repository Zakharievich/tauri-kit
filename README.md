...
**3. Установите и запустите** приложение, затем заполните поля:
- **URL сервера** — `https://ваш-домен` (без `/token` в конце);
- **Имя комнаты** и **Ваше имя**;
- при необходимости — опции (E2EE / транскрибация).

Для конференции на 2+ человек у всех участников должны совпадать «URL сервера» и «Имя комнаты».
